...
import com.bot.game.service.impl.AddAttributeServiceImpl;
 ...
 */
public class AddResetAttributePrinter extends Menu {

    private final PlayerPhantom playerPhantom;

    private final GoodsDetailDTO goodsDetailDTO;

    AddResetAttributePrinter(PlayerPhantom playerPhantom, GoodsDetailDTO goodsDetailDTO) {
    ...
    public void getDescribe(String token) {
        ...
        this.describe = GameConsts.MyKnapsack.CHOOSE_ATTRIBUTE_2;
        int index = 1;
        for (ENPhantomAttribute enPhantomAttribute : ENPhantomAttribute.values()) {
            ...
            if (number > 1) {
                this.playServiceMap.put(IndexUtil.getIndex(index), new AddAttributeServiceImpl(playerPhantom, enPhantomAttribute, goodsDetailDTO));
                index++;
            }
        }
